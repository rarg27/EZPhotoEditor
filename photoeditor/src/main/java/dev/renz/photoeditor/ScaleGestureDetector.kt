...
class ScaleGestureDetector(private val mListener : OnScaleGestureListener) {
    ...
    open class SimpleOnScaleGestureListener : OnScaleGestureListener {
        ...
        override fun onScale(view: View, detector: ScaleGestureDetector): Boolean = false

        override fun onScaleBegin(view: View, detector: ScaleGestureDetector): Boolean = true
    ...
    private var mCurrSpanVector: Vector2D = Vector2D()
        ...
        if (mInvalidGesture) {
            ...
            when (action) {
                MotionEvent.ACTION_POINTER_DOWN -> {
                    ...
                    val oldActive0: Int = mActiveId0
                    val oldActive1: Int = mActiveId1
                    reset()
                    mPrevEvent = MotionEvent.obtain(event)
                ...
                MotionEvent.ACTION_POINTER_UP -> {
                    ...
                    if (pointerCount > 2) {
                        if (actionId == mActiveId0) {
                            val newIndex: Int = findNewActiveIndex(event, mActiveId1, actionIndex)
                            if (newIndex >= 0) {
                                mListener.onScaleEnd(view, this)
                            ...
                            val newIndex: Int = findNewActiveIndex(event, mActiveId0, actionIndex)
                            if (newIndex >= 0) {
                                mListener.onScaleEnd(view, this)
                    ...
                    if (gestureEnded) {
                        ...
                        val activeId: Int = if (actionId == mActiveId0) mActiveId1 else mActiveId0
                        val index: Int = event.findPointerIndex(activeId)
